...
-- Step 3: Upsert RPC — called from the public widget
-- Finds or creates the subscriber, then links to the sumula (idempotent).
...
DECLARE
  v_sub_id uuid;
BEGIN
  -- Find existing subscriber by e-mail or whatsapp
  ...
  ELSIF NULLIF(trim(coalesce(p_name, '')), '') IS NOT NULL THEN
    ...
    WHERE id = v_sub_id AND name IS NULL;
  END IF;

  -- Link to sumula (idempotent)
